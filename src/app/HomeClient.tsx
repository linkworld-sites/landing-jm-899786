...
import type { FaqEntry } from "@/lib/site-meta";
...
// ── FAQ — answers the real buyer questions (cost, process, timeline) so
// search/AI engines can cite them, mirrored 1:1 by the FAQPage JSON-LD ─────

function FaqSection({ faq }: { faq: FaqEntry[] }) {
  if (faq.length === 0) return null;
  return (
    <section className="relative z-10 -mt-16 rounded-t-[2.5rem] bg-paper py-24 shadow-[0_-40px_80px_-40px_rgba(0,0,0,0.45)] md:-mt-24 md:py-32">
      <div className="mx-auto max-w-3xl px-6 md:px-12 lg:px-20">
        <FadeUp className="mb-14 max-w-xl md:mb-16">
          ...
          <h2 className="font-display font-light text-ink leading-[0.95] text-[clamp(2.5rem,6vw,4.5rem)]">
            Kurz beantwortet.
          </h2>
        </FadeUp>

        <div className="divide-y divide-ink/10 rounded-2xl border border-ink/10 px-6 md:px-8">
          {faq.map((item) => (
            <div key={item.q} className="py-7 md:py-8">
              <h3 className="mb-2.5 font-label text-base font-semibold text-ink md:text-lg">
                {item.q}
              </h3>
              <p className="font-label text-sm leading-relaxed text-ink/60 md:text-base">
                {item.a}
              </p>
            </div>
          ))}
...
export default function HomeClient({ faq }: { faq: FaqEntry[] }) {
  return (
    <>
      <HeroSection />
      <FeaturesSection />
